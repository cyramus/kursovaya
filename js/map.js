...
class MapPage {
    ...
    enrichVenuesWithDayInfo() {
        // Обогащаем данные о мероприятиях информацией о дне из events.json
        Object.values(VENUES_DATA).forEach(venue => {
            venue.events.forEach(event => {
                // Ищем соответствующее событие в events.json по названию
                const matchedEvent = this.eventsData.find(e => {
                    // Сопоставляем по названию и времени
                    const timeMatch = `${e.startTime}–${e.endTime}` === event.time;
                    const titleMatch = e.title === event.title;
                    return titleMatch || (timeMatch && e.venue.includes(venue.name.split(',')[0]));
                });
                if (matchedEvent) {
                    event.day = matchedEvent.day;
                }
            });
        });
    }
    ...
    setupEventListeners() {
        ...
        if (navToggle && navList) {
            navToggle.addEventListener('click', () => {
                const isOpen = navList.classList.toggle('nav__list_open');
                ...
                <div class="venue-details__events">
                    ...
                    <ul class="venue-details__events-list">
                        ${venue.events.map(event => {
                            ...
                            const dayLabel = event.day ? dayLabels[event.day] : '';
                            const timeWithDay = event.day ? `${dayLabel}, ${event.time}` : event.time;
                            return `
                            <li class="venue-details__event-item">
                                <span class="venue-details__event-name">${event.title}</span>
                                <span class="venue-details__event-time">${timeWithDay}</span>
                            </li>
                        `;
                        }).join('')}
                    </ul>
