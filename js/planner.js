class PlannerPage {
    ...
    switchDay(day) {
        ...
        tabs.forEach(tab => {
            if (tab.dataset.day === day) {
                tab.classList.add('planner-schedule__tab_active');
                tab.setAttribute('aria-selected', 'true');
            } else {
                tab.classList.remove('planner-schedule__tab_active');
    ...
    renderEvent(event, nextEvent) {
        ...
        return `
            <div class="planner-event ${conflicts.length > 0 ? 'planner-event_conflict' : ''}">
                ...
                ` : ''}
                ${timeToNext !== null ? `
                    <div class="planner-event__next ${hasTimeWarning ? 'planner-event__next_warning' : ''}">
